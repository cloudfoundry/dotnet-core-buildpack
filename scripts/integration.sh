...
BUILDPACK_FILE="$UNCACHED_BUILDPACK_FILE" \
  ginkgo -mod=vendor -r --compilers=1 --flakeAttempts=$GINKGO_ATTEMPTS -nodes $GINKGO_NODES --slowSpecThreshold=60 -- --cached=false

echo "Run Cached Buildpack"
BUILDPACK_FILE="$CACHED_BUILDPACK_FILE" \
  ginkgo -mod=vendor -r --compilers=1 --flakeAttempts=$GINKGO_ATTEMPTS -nodes $GINKGO_NODES --slowSpecThreshold=60 -- --cached
